...
namespace Offline_lendingmanagementsystem_Its
{
    public partial class dialogsearch : Form
    {
        ...
        public void display()
        {
            ...
            string strAcct = "Select * from customerinfo where No like '%" + borrow.search.Replace("'", "''") + "%' || concat (LastName,' ',FirstName,' ',MiddleName) like '%" + borrow.search.Replace("'", "''") + "%'";
        ...
        {
            borrow.id = dataGridView1.Rows[dataGridView1.CurrentRow.Index].Cells[0].Value.ToString();
